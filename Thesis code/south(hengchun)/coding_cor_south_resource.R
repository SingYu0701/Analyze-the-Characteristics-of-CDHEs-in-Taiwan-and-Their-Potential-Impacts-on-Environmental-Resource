...
results_selected2 <- results %>%
  filter(
    (System == "Reservoir" & Effect %in% c("前一年事件 × 年變化")) |
      ...
      (System == "Aquaculture" & Effect %in% c("前一年事件 × 當年偏差","當年事件 × 當年偏差")) |
      (System == "Electricity" & Effect == "前一年事件 × 當年偏差") |
      (System == "Heat-related cases" & Effect == "當年事件 × 年變化")
  ) %>%
...
ggplot(results_selected2, aes(x = reorder(System, Correlation), y = Correlation, fill = Effect_Type)) +
  ...
  ylab("Correlation") +
  xlab("System") +
  ggtitle("(b) Effects of Compound Drought–Heat Events on Resource Systems in Hengchun") +
